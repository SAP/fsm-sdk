...
const current = Object.keys(ALL_DTO_VERSIONS)
...
(async () => {
    ...
    await page
        .waitForSelector('.topictitle')

    await page.evaluate((data) => {
        eval(`
        let dtos = jQuery('.topictitle').get()
            .filter(it => jQuery(it).text().includes(' DTO '))
            .map(x => jQuery(x).text().trim())
            .reduce((it, value) => {
