...
describe('DataApi', () => {
  ...
  it('should throw on unknown DTOs', done => {
    client.dataServiceAPI.getById('THIS_IS_NOT_A_DTO' as any, 'SOME-ID')
      .catch(error => {
  ...
  it('POST serviceCall Id', done => {
    ...
    client.dataServiceAPI.post('ServiceCall', serviceCall)
      .then(fromDB => {
  ...
  it('POST serviceCall ExternalId', done => {
    ...
    client.dataServiceAPI.postByExternalId('ServiceCall', serviceCall)
      .then(fromDB => {
        const [{ serviceCall }] = fromDB.data;
  ...
  it('GET serviceCall by Id', done => {
    ...
    client.dataServiceAPI.getById<{ id: string }>('ServiceCall', THE_ID)
      .then(fromDB => {
        const [{ serviceCall }] = fromDB.data;
        assert.strictEqual(serviceCall.id, THE_ID, 'POST');
        return serviceCall;
  ...
  it('GET serviceCall by ExternalId', done => {
    Promise.all([
      client.dataServiceAPI.getById<{ id: string }>('ServiceCall', THE_ID),
      client.dataServiceAPI.getByExternalId<{ id: string }>('ServiceCall', THE_EXTERNAL_ID)
    ])
      .then(([resp1, resp2]) => {
  ...
  it('PUT serviceCall by Id', done => {
    client.dataServiceAPI.getById<{ id: string, subject: string }>('ServiceCall', THE_ID)
      .then(fromDB => {
        const [{ serviceCall }] = fromDB.data;
        return client.dataServiceAPI.put('ServiceCall', { ...serviceCall, subject: 'subject-changed-with-PUT-ID' })
      })
      .then(response => {
  ...
  it('PUT serviceCall by ExternalId', done => {
    client.dataServiceAPI.getByExternalId<{ id: string, subject: string, lastChanged: number }>('ServiceCall', THE_EXTERNAL_ID)
      .then(fromDB => {
        const [{ serviceCall }] = fromDB.data;
        return client.dataServiceAPI.putByExternalId('ServiceCall', { externalId: THE_EXTERNAL_ID, lastChanged: serviceCall.lastChanged, subject: 'subject-changed-with-PUT-ExternalId' })
      })
      .then(response => {
        const [{ serviceCall }] = response.data;
  ...
  it('PATCH serviceCall by Id', done => {
    client.dataServiceAPI.getById<{ id: string, lastChanged: number }>('ServiceCall', THE_ID)
      .then(async (fromDB) => {
        const [{ serviceCall }] = fromDB.data;
        return client.dataServiceAPI.patch('ServiceCall', {
          id: THE_ID,
  ...
  it('PATCH serviceCall by ExternalId', done => {
    client.dataServiceAPI.getByExternalId<{ id: string, lastChanged: number, }>('ServiceCall', THE_EXTERNAL_ID)
      .then(async (fromDB) => {
        const [{ serviceCall }] = fromDB.data;
        return client.dataServiceAPI.patchByExternalId('ServiceCall', {
          externalId: THE_EXTERNAL_ID,
  ...
  it('DELETE serviceCall by Id', done => {
    client.dataServiceAPI.getById<{ id: string, lastChanged: number }>('ServiceCall', THE_ID)
      .then(fromDB => {
        const [{ serviceCall }] = fromDB.data;
        return client.dataServiceAPI.deleteById('ServiceCall', { id: THE_ID, lastChanged: serviceCall.lastChanged as number })
      })
      .then(_ => client.dataServiceAPI.getById('ServiceCall', THE_ID))
  ...
  it('DELETE serviceCall by externalId', done => {
    ...
    client.dataServiceAPI.post<{ externalId: string, lastChanged: number }>('ServiceCall', serviceCall)
      .then(response => {
        const [{ serviceCall }] = response.data;
        return client.dataServiceAPI.deleteByExternalId('ServiceCall', { externalId: serviceCall.externalId, lastChanged: serviceCall.lastChanged as number })
      })
      .then(_ => client.dataServiceAPI.getById('ServiceCall', id))
      .then(fromDB => assert.strictEqual(fromDB.data.length, 0))
      .then(_ => done())
      .catch(e => done(e));
  }).timeout(ClientConfigBuilder.getTestTimeout());
  ...
  it('External Id E2E', done => {
    ...
    client.dataServiceAPI.batch([
      ...
      .then(async () => {
        ...
        const [{ serviceCall }] = await client.dataServiceAPI.getByExternalId<{ id: string, lastChanged: number, businessPartner: { id: string, externalId: string } }>('ServiceCall', SC_EXT_ID).then(x => x.data);
      ...
      // clean up
      .then(() => client.dataServiceAPI.batch<{}>([
